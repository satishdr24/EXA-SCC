...
*      DATA(lv_billing_period) =   |{ <royalty>-ryear }/| & |{ <royalty>-poper }| .
      DATA(lv_billing_period) = |{ <royalty>-poper }.| & |{ <royalty>-ryear }|.

      DATA(lv_srv_prod_s) =   |{ <royalty>-license }| & | ({ <royalty>-LicenseDescription })|.
*      CONCATENATE 'Royalty ||' lv_legal_rec '||'  lv_billing_period '||'  lv_legal_s '||'  lv_srv_prod_s
*                        INTO  ev_header SEPARATED BY space .

      CONCATENATE 'Royalty Calculation ||' lv_billing_period '|| from'
                  <royalty>-licensorlegalentity
                  'to'
                  <royalty>-licenseelegalentity '||'  lv_srv_prod_s
                  INTO  ev_header SEPARATED BY space .
    ENDLOOP.
  ENDMETHOD.
