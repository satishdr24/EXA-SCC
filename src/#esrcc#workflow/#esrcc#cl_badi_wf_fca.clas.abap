CLASS /esrcc/cl_badi_wf_fca DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS /esrcc/cl_badi_wf_fca IMPLEMENTATION.
...
*check workflow config for group by
    ...
    CASE workflowswitch-workflowgroupby.
      ...
      WHEN /esrcc/cl_wf_utility=>c_groupby-receiver.
        /esrcc/cl_wf_utility=>set_fcareceiver_task_descr(
          EXPORTING
            it_leading_object = it_leading_object
          CHANGING
            et_task_desc      = et_task_desc
...
*check workflow config for group by
    ...
    IF sy-subrc EQ 0.
      lv_poper = ls_leading_object-refpoper + 1.
...
* Description should be like
    ...
    CONCATENATE 'Forecast Charge-out ||' lv_header
        ...
        SELECT DISTINCT
          ...
          FOR ALL ENTRIES IN @it_leading_object
               WHERE costbase~fplv        = @it_leading_object-fplv  AND
                   ...
                   costbase~processtype   = @/esrcc/if_calculate_chargeout=>forecastprocesstype AND
