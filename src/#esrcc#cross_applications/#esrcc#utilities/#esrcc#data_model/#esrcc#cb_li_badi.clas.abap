...
CLASS /esrcc/cb_li_badi IMPLEMENTATION.
  ...
  METHOD /esrcc/if_dao_badi~determine_data.
    ...
    SELECT
      FROM /esrcc/cstelemnt AS header
             INNER JOIN
               /esrcc/cstelmtch AS matching ON matching~sysid = header~sysid AND
                                               matching~cost_element = header~cost_element
      FIELDS header~sysid,
      ...
      WHERE header~sysid          = @_line_items-sysid
        AND matching~legal_entity = @_line_items-legalentity
        AND matching~company_code = @_line_items-ccode
        AND matching~cost_object  = @_line_items-costobject
        AND matching~cost_center  = @_line_items-costcenter
...
*    DELETE model FROM missing_data.
    ...
    SELECT
      FROM @model AS cb_li
             LEFT OUTER JOIN
               /esrcc/cstelemnt AS cost_element ON cb_li~costelement = cost_element~cost_element
      FIELDS *
