...
CLASS /esrcc/cl_download_template IMPLEMENTATION.
  METHOD if_sadl_exit_calc_element_read~calculate.
    ...
    LOOP AT original_data ASSIGNING FIELD-SYMBOL(<original_data>).
      <original_data>-TableName = SWITCH tabname( <original_data>-SubApplication
      ...
      IF template_helper->check_table_registered( <original_data>-TableName ).
...
*      DATA(components) = lcl_components=>get_components( <original_data>-TableName ).
*
      "To derive the components of include/append structure
      LOOP AT components ASSIGNING FIELD-SYMBOL(<incl_component>) WHERE as_include = abap_true.
        APPEND LINES OF CAST cl_abap_structdescr( cl_abap_typedescr=>describe_by_name(
                                                           to_upper( <incl_component>-type->get_relative_name( ) ) ) )->get_components( ) TO components.
*
      ENDLOOP.

      DELETE components WHERE name = 'CLIENT' OR name = 'CREATED_BY' OR name = 'CREATED_AT' OR name = 'LAST_CHANGED_BY'
         OR name = 'LAST_CHANGED_AT' OR name = 'LOCAL_LAST_CHANGED_AT' OR as_include = abap_true.
