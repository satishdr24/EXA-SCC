...
*          AND legalentity = @cost_bases-legalentity AND ccode      = @cost_bases-ccode      AND belnr = @cost_bases-belnr
*          AND buzei       = @cost_bases-buzei       AND costobject = @cost_bases-costobject
...
*                                                                 AND rr_li~reference_belnr    = uploaded_rr_lineitems~reference_belnr
*                                                                 AND rr_li~status NOT IN ( 'A' , 'E' )
...
*
*      " Validating Status: Finalized should not be allowed to upload
*      IF     <ryear>           IS ASSIGNED
*         AND <poper>           IS ASSIGNED
*         AND <fplv>            IS ASSIGNED
*         AND <sysid>           IS ASSIGNED
...
*         AND <reference_belnr> IS ASSIGNED.
*
...
*                                                                     rr_matter       = <rr_matter>
*                                                                     material        = <material>
*                                                                     reference_belnr = <reference_belnr> ] ).
*          is_valid_entry = abap_false.
*          _application_logger->add_message(
*              log_message    = VALUE #( message_id      = /esrcc/data_upload_util=>message_class
*                                        message_type    = 'E'
