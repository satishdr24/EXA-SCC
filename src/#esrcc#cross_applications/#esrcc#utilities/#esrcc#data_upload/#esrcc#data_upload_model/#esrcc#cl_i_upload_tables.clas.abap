...
*          sql_filter_string = |( { sql_filter_string } AND { search_expression_sql } )|.
          sql_filter_string = |( { sql_filter_string } )|.
        ENDIF.
        " request data
...
* This entry is not relevant as Table name is now optional
*        APPEND VALUE #( tablename   = |Scenario's Data|
*                            language    = sy-langu
*                            description = 'Use with other scenarios except Business Configuration.'(001) ) TO results.
        ...
        IF sort_elements IS NOT INITIAL.
          DATA final_result type TABLE OF /ESRCC/I_UploadTables.
          SELECT (requested_element)
                 FROM @results AS result
                 WHERE (sql_filter_string)
                 ORDER BY (sort_string)
                 INTO CORRESPONDING FIELDS OF TABLE @final_result
                 OFFSET @offset UP TO @max_rows ROWS.

        ENDIF.

        io_response->set_data( final_result ).
        " request count
