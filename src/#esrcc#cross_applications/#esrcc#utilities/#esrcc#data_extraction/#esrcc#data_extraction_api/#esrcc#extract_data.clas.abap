...
CLASS /esrcc/extract_data IMPLEMENTATION.
  ...
  METHOD /esrcc/if_extract_data~extract.
    ...
    ASSIGN _filters[ filter_name = 'REP_YEAR' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      DATA(year) = CONV /esrcc/ryear( <filter_value>[ 1 ]-low  ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'PERIOD' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      validity = CORRESPONDING #( <filter_value> ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'PST_DATE' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      DATA(posting_date) = CONV /esrcc/postingdate( <filter_value>[ 1 ]-low ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'EXT_OBJ' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      DATA(object) = CONV /esrcc/extraction_object( <filter_value>[ 1 ]-low ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'LEDGER' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      DATA(ledger) = CONV /esrcc/ledger_de( <filter_value>[ 1 ]-low ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'ENTITY' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      legal_entities = CORRESPONDING #( <filter_value> ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'CSTCENTR' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      cost_centers = CORRESPONDING #( <filter_value> ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'CSTELEM' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      cost_elements = CORRESPONDING #( <filter_value> ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'PKG_CODE' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      package_codes = CORRESPONDING #( <filter_value> ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'PKG_SIZE' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
      DATA(package_size) = CONV i( <filter_value>[ 1 ]-low ).
    ENDIF.

    ASSIGN _filters[ filter_name = 'SIMULATE' ]-filter_value->* TO <filter_value> ELSE UNASSIGN.
    IF sy-subrc = 0.
