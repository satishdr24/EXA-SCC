CLASS /esrcc/cl_application_logs DEFINITION PUBLIC FINAL CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    DATA :_serial_no_max TYPE /esrcc/serial_no.
ENDCLASS.
...
CLASS /esrcc/cl_application_logs IMPLEMENTATION.
  ...
  METHOD /esrcc/if_application_logs~add_message.
    ...
    "Fetching the Latest Serial No
    IF _serial_no_max IS INITIAL.
  ...
  METHOD /esrcc/if_application_logs~add_messages.
    ...
    TRY.
        APPEND LINES OF VALUE _log_model_type-items(
            FOR <msg> IN log_messages INDEX INTO serial_index
            ( VALUE #(
                  ...
                  log_uuid        = COND #( WHEN <msg>-log_uuid IS INITIAL
                                            ...
                                            ELSE <msg>-log_uuid )
                  log_header_uuid = _logs-header-log_header_uuid
                  serial_number   =  _serial_no_max + serial_index
                  created_at      = COND #( WHEN <msg>-created_at IS INITIAL THEN timestamp ELSE <msg>-created_at ) ) )
                   ) TO _logs-items.


        _serial_no_max = VALUE #( _logs-items[ lines( _logs-items ) ]-serial_number OPTIONAL ).
