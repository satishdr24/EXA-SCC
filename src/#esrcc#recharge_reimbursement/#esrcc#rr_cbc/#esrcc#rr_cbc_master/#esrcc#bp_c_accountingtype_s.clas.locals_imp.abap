CLASS LHC_/ESRCC/I_ACCOUNTINGTYPE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE AccountingTypeAll\_AccountingType
          ENTITIES_UPDATE FOR UPDATE AccountingType.
ENDCLASS.

CLASS LHC_/ESRCC/I_ACCOUNTINGTYPE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_RR_Accounting_Type\_AccountingTypeText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_RR_Accounting_Type\_AccountingTypeText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_AccountingTypeText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_RR_Accounting_Type\_AccountingTypeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-AccountingType = <target>-%KEY-AccountingType
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_AccountingType_S
      ENTITY AccountingType
        CREATE BY \_AccountingTypeText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_AccountingType_S
