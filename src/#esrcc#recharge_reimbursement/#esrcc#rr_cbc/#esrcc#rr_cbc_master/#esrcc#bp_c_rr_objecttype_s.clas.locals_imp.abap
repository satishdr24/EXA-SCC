CLASS LHC_/ESRCC/I_RR_OBJECTTYPE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE ObjectTypeAll\_ObjectType
          ENTITIES_UPDATE FOR UPDATE ObjectType.
ENDCLASS.

CLASS LHC_/ESRCC/I_RR_OBJECTTYPE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_RR_ObjectType\_ObjectTypeText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_RR_ObjectType\_ObjectTypeText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_RR_ObjectTypeText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_RR_ObjectType\_ObjectTypeText,
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_RR_ObjectType_S
      ENTITY ObjectType
        CREATE BY \_ObjectTypeText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_RR_ObjectType_S
