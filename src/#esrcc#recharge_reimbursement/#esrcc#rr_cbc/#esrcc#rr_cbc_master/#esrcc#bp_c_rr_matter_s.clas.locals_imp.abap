CLASS LHC_/ESRCC/I_RR_MATTER_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE RRMatterAll\_RRMatter
          ENTITIES_UPDATE FOR UPDATE RRMatter.
ENDCLASS.

CLASS LHC_/ESRCC/I_RR_MATTER_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_RR_Matter\_RRMatterText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_RR_Matter\_RRMatterText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_RR_MatterText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_RR_Matter\_RRMatterText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-RrMatter = <target>-%KEY-RrMatter
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_RR_Matter_S
      ENTITY RRMatter
        CREATE BY \_RRMatterText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_RR_Matter_S
