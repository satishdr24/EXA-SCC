...
CLASS /esrcc/cl_badi_execcockpit IMPLEMENTATION.
  ...
  METHOD /esrcc/if_execcockpit~background_scheduler.
    ...
    DATA lo_forecast_badi TYPE REF TO /esrcc/badi_forecast.
    ...
    IF sy-subrc = 0.
      ...
      IF lo_forecast_badi IS NOT BOUND.
        TRY.
            GET BADI lo_forecast_badi.
          CATCH cx_badi_not_implemented cx_badi_unknown_error.
        ENDTRY.
      ENDIF.
      ...
      CASE <ls_parameters>-low.
        ...
        WHEN /esrcc/if_calculate_chargeout=>calculate_forecast.
          IF lo_forecast_badi IS BOUND.

            CALL BADI lo_forecast_badi->calculate_forecast
              EXPORTING
                it_keys = lt_keys .

          ENDIF.
        WHEN /esrcc/if_calculate_chargeout=>finalize_forecast.
          IF lo_forecast_badi IS BOUND.

            CALL BADI lo_forecast_badi->finalize_forecast
              EXPORTING
                it_keys = lt_keys.

          ENDIF.
        WHEN /esrcc/if_calculate_chargeout=>reopen_forecast.
          IF lo_forecast_badi IS BOUND.

            CALL BADI lo_forecast_badi->reopen_forecast
              EXPORTING
                it_keys = lt_keys.
