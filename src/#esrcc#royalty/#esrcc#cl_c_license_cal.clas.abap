...
***select data
        ...
        DATA lt_count         TYPE STANDARD TABLE OF /esrcc/c_licencse_cal.
        ...
        DATA _licensetype     TYPE RANGE OF /esrcc/licensetype.
        DATA _royaltybasekey  TYPE RANGE OF /esrcc/royaltybasekeys.
        DATA _royaltycomputationmethod TYPE RANGE OF /esrcc/royaltycomputationmeth.
...
*   filters
        LOOP AT lt_filter ASSIGNING FIELD-SYMBOL(<ls_filter>).
          ...
          CASE <ls_filter>-name.
            ...
            WHEN 'LICENSEtyp'.
              _licensetype = CORRESPONDING #( <ls_filter>-range ).
            WHEN 'ROYALTYBASEKEY'.
              _royaltybasekey = CORRESPONDING #( <ls_filter>-range ).
            WHEN 'ROYALTYCOMPUTATIONMETHOD'.
              _royaltycomputationmethod = CORRESPONDING #( <ls_filter>-range ).
            WHEN OTHERS.
          ENDCASE.
        ...
        SELECT DISTINCT
          ...
          FROM /esrcc/i_read_royalty AS royalcal
              WHERE royalcal~LicensorCCode IN @_ccode
                ...
                AND royalcal~licensetyp          IN @_licensetype
                ...
                AND royalcal~royaltybasekey      IN @_royaltybasekey
                AND royalcal~royaltycomputationmethod IN @_royaltycomputationmethod
...
*                 WHERE active = @abap_true
                  WHERE LicensorCCode IN @_ccode
                   ...
                   AND LicenseTyp          IN @_licensetype
                   ...
                   AND royaltybasekey      IN @_royaltybasekey
                   AND royaltycomputationmethod IN @_royaltycomputationmethod
                   AND Ryear            IN @_ryear
                   AND poper            IN @_poper
                   AND fplv             IN @_fplv
*                 WHERE (lv_sql_filter)
