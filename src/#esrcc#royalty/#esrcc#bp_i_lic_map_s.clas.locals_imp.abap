...
CLASS lcl_custom_validation IMPLEMENTATION.
  ...
  METHOD precheck_cba_mapping.
    ...
    SELECT
           map~licensorsysid,
           map~licensorlegalentity,
           map~licensorcompanycode,
           map~licensorcostobject,
           map~licensorcostcenter,
           map~license
        FROM /esrcc/d_lic_map AS map
        INNER JOIN @target_entities AS tent
            ON  tent~licensorsysid       = map~licensorsysid
            AND tent~licensorlegalentity = map~licensorlegalentity
            AND tent~licensorcompanycode = map~licensorcompanycode
            AND tent~licensorcostobject  = map~licensorcostobject
            AND tent~licensorcostcenter  = map~licensorcostcenter
            AND tent~license             = map~license
        WHERE map~draftentityoperationcode NOT IN ( 'D', 'L' )
        INTO TABLE @DATA(duplicate_entities).

    LOOP AT target_entities INTO DATA(entity) GROUP BY ( licensorsysid       = entity-licensorsysid
                                                         licensorlegalentity = entity-licensorlegalentity
                                                         licensorcompanycode = entity-licensorcompanycode
                                                         licensorcostobject  = entity-licensorcostobject
                                                         licensorcostcenter  = entity-licensorcostcenter
                                                         license             = entity-license
                                                         size                = GROUP SIZE )
        ASCENDING REFERENCE INTO DATA(group_ref).

*     Validate UUID
      IF /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( group_ref->*
                                                                               MAPPING sysid       = licensorsysid
                                                                                       legalentity = licensorlegalentity
                                                                                       companycode = licensorcompanycode
                                                                                       costobject  = licensorcostobject
                                                                                       costcenter  = licensorcostcenter ) ) IS INITIAL.
        lo_lic_map->set_invalid_group_set(
          linked_fields = VALUE #( ( 'LICENSORSYSID' )
                                   ( 'LICENSORLEGALENTITY' )
                                   ( 'LICENSORCOMPANYCODE' )
                                   ( 'LICENSORCOSTOBJECT' )
                                   ( 'LICENSORCOSTCENTER' ) )
          entity        = CORRESPONDING ts_lic_map( group_ref->* ) ).
        CONTINUE.
      ENDIF.

      IF line_exists( duplicate_entities[ licensorsysid       = group_ref->licensorsysid
                                          licensorlegalentity = group_ref->licensorlegalentity
                                          licensorcompanycode = group_ref->licensorcompanycode
                                          licensorcostobject  = group_ref->licensorcostobject
                                          licensorcostcenter  = group_ref->licensorcostcenter
...
CLASS lhc_rap_tdat_cts DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-DATA:
        c_tech_id TYPE /esrcc/smbc_technical_id VALUE '/ESRCC/LIC_MAP'.
ENDCLASS.
...
CLASS lhc_/esrcc/i_licmap DEFINITION FINAL INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS setuuid FOR DETERMINE ON SAVE
      IMPORTING keys FOR licensee~setuuid.
ENDCLASS.
...
*     Validate UUID
      IF /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( entity
                                                                               MAPPING sysid       = licenseesysid
                                                                                       legalentity = licenseelegalentity
                                                                                       companycode = licenseecompanycode
                                                                                       costobject  = licenseecostobject
                                                                                       costcenter  = licenseecostcenter ) ) IS INITIAL.
        lo_config_util->set_invalid_group_set(
          linked_fields = VALUE #( ( 'LICENSEESYSID' )
                                   ( 'LICENSEELEGALENTITY' )
                                   ( 'LICENSEECOMPANYCODE' )
                                   ( 'LICENSEECOSTOBJECT' )
                                   ( 'LICENSEECOSTCENTER' ) )
      ...
      IF entity-licenseesysid       = entity-licensorsysid AND
         entity-licenseelegalentity = entity-licensorlegalentity AND
         entity-licenseecompanycode = entity-licensorcompanycode AND
         entity-licenseecostobject  = entity-licensorcostobject AND
         entity-licenseecostcenter  = entity-licensorcostcenter.
  ...
  METHOD setuuid.
    ...
    MODIFY ENTITIES OF /esrcc/i_licmap_s IN LOCAL MODE
      ENTITY licensee
      UPDATE FIELDS ( licensorcostobjectuuid licenseecostobjectuuid )
      WITH VALUE #( FOR entity IN entities ( %tky = entity-%tky
                                             licensorcostobjectuuid = /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( entity
                                                                                                                                            MAPPING sysid       = licensorsysid
                                                                                                                                                    legalentity = licensorlegalentity
                                                                                                                                                    companycode = licensorcompanycode
                                                                                                                                                    costobject  = licensorcostobject
                                                                                                                                                    costcenter  = licensorcostcenter ) )
                                             licenseecostobjectuuid = /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( entity
                                                                                                                                            MAPPING sysid       = licenseesysid
                                                                                                                                                    legalentity = licenseelegalentity
                                                                                                                                                    companycode = licenseecompanycode
                                                                                                                                                    costobject  = licenseecostobject
                                                                                                                                                    costcenter  = licenseecostcenter ) ) ) )
      FAILED DATA(ufailed)
      REPORTED DATA(ureported).

    reported-licensee = CORRESPONDING #( ureported-licensee ).
  ENDMETHOD.
