...
*        costelemtype TYPE if_abap_behv=>t_xflag,
      END OF ts_control.
...
*    IF control-costelemtype = if_abap_behv=>mk-on. APPEND VALUE #( fieldname = 'COSTELEMTYPE' ) TO fields. ENDIF.
...
*          lo_cost_element->set_invalid_group_set(
*            linked_fields = VALUE #( ( 'SYSID' )
*                                     ( 'LEGALENTITY' )
*                                     ( 'COMPANYCODE' ) )
*            entity        = CORRESPONDING ts_cost_element( group_ref->* ) ).
*        ENDIF.
*
...
CLASS lhc_rap_tdat_cts IMPLEMENTATION.
  METHOD get.
    result = mbc_cp_api=>rap_tdat_cts( tdat_name = '/ESRCC/COSTELEMENTS'
                                       table_entity_relations = VALUE #( ( entity = 'CostElement' table = '/ESRCC/CSTELEMNT' )
                                                                         ( entity = 'CostElementsText' table = '/ESRCC/CSTELEMTT' ) ) ) ##NO_TEXT.
  ENDMETHOD.
ENDCLASS.
    ...
    LOOP AT ref_main ASSIGNING FIELD-SYMBOL(<ref_main>).
      ...
      APPEND VALUE #(
        ...
        %target = VALUE #( ( %cid        = key_cid
                             %is_draft   = <ref_main>-%is_draft
                             %data       = CORRESPONDING #( <ref_main> EXCEPT sysid costelement singletonid )
...
*                             legalentity = key-%param-legalentity
      ...
      LOOP AT ref_text ASSIGNING FIELD-SYMBOL(<ref_text>) USING KEY draft WHERE %tky-%is_draft       = key-%tky-%is_draft
                                                                            ...
                                                                            AND %tky-CostElement     = key-%tky-CostElement.
        ...
        INSERT VALUE #( %cid      = key_cid && <ref_text>-spras
                        ...
                        %data     = CORRESPONDING #( <ref_text> EXCEPT sysid costelement singletonid ) ) INTO TABLE <new_text>-%target.
      ENDLOOP.
    ENDLOOP.
