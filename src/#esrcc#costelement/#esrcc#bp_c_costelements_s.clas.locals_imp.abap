CLASS LHC_/ESRCC/I_COSTELEMENTS_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE CostElementsAll\_CostElements
          ENTITIES_UPDATE FOR UPDATE CostElements.
ENDCLASS.

CLASS LHC_/ESRCC/I_COSTELEMENTS_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_CostElements\_CostElementsText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_CostElements\_CostElementsText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_CostElementsText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_CostElements\_CostElementsText,
...
*                      %KEY-CostElementUuid = <target>-%KEY-CostElementUuid
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_CostElements_S
      ENTITY CostElement
        CREATE BY \_CostElementsText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_CostElements_S
