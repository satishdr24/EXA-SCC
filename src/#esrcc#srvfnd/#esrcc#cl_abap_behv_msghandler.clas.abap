CLASS /esrcc/cl_abap_behv_msghandler DEFINITION
  ...
  PROTECTED SECTION.
    TYPES:
        tt_linked_fields TYPE STANDARD TABLE OF abp_field_name.

    DATA:
      gt_linked_fields TYPE tt_linked_fields.
  ...
  PRIVATE SECTION.
    ...
    METHODS set_element_field_error
      IMPORTING
        element_field TYPE abp_field_name
      CHANGING
        reported      TYPE any.
...
CLASS /esrcc/cl_abap_behv_msghandler IMPLEMENTATION.
  ...
  METHOD set_element_field_error.       " Set error on field
    IF element_field IS INITIAL OR gv_is_transition = abap_true.
      RETURN.
    ENDIF.

    ASSIGN COMPONENT cl_abap_behv=>co_techfield_name-element OF STRUCTURE reported TO FIELD-SYMBOL(<elements>).
    IF sy-subrc = 0.
      ASSIGN COMPONENT element_field OF STRUCTURE <elements> TO FIELD-SYMBOL(<field>).
      IF sy-subrc = 0.
...
*   Set error on field
    IF gv_is_transition = abap_false.
      IF gt_linked_fields IS INITIAL.
        set_element_field_error( EXPORTING element_field = gs_field-fieldname CHANGING reported = <reported_entity> ).
      ELSE.
        LOOP AT gt_linked_fields INTO DATA(linked_field).
          set_element_field_error( EXPORTING element_field = linked_field CHANGING reported = <reported_entity> ).
        ENDLOOP.
      ENDIF.
    ENDIF.
  ENDMETHOD.
