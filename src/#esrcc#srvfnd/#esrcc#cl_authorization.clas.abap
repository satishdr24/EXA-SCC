  ...
  PUBLIC SECTION.
    ...
    METHODS regulate_action_finalize
      IMPORTING
        is_draft            TYPE abp_behv_flag OPTIONAL
        wf_status           TYPE /esrcc/status_de
        wf_internal_status  TYPE /esrcc/status_de OPTIONAL
      RETURNING
        VALUE(is_regulated) TYPE if_abap_behv=>t_xflag.
    ...
    CLASS-METHODS: check_auth_upload RETURNING VALUE(is_authorised) TYPE abap_bool.
    CLASS-METHODS: check_auth_download RETURNING VALUE(is_authorised) TYPE abap_bool.
    CLASS-METHODS: check_auth_create RETURNING VALUE(is_authorised) TYPE abap_bool.
    CLASS-METHODS: check_auth_execute RETURNING VALUE(is_authorised) TYPE abap_bool.
...
CLASS /esrcc/cl_authorization IMPLEMENTATION.
  ...
  METHOD regulate_action_finalize.
    ...
    IF wf_internal_status IS NOT INITIAL.
      is_regulated = if_abap_behv=>fc-o-disabled.
    ENDIF.
  ENDMETHOD.
...
*   Set instance authorization
  ...
  METHOD check_auth_download.
    AUTHORITY-CHECK OBJECT '/ESRCC/ADM' ID 'ACTVT' FIELD 'DL'.
    is_authorised = COND #( WHEN sy-subrc = 0 THEN abap_true ELSE abap_false ).
  ENDMETHOD.

  METHOD check_auth_upload.
    AUTHORITY-CHECK OBJECT '/ESRCC/ADM' ID 'ACTVT' FIELD 'UL'.
    is_authorised = COND #( WHEN sy-subrc = 0 THEN abap_true ELSE abap_false ).
  ENDMETHOD.

  METHOD check_auth_create.
    AUTHORITY-CHECK OBJECT '/ESRCC/ADM' ID 'ACTVT' FIELD '01'.
    is_authorised = COND #( WHEN sy-subrc = 0 THEN abap_true ELSE abap_false ).
  ENDMETHOD.

  METHOD check_auth_execute.
    AUTHORITY-CHECK OBJECT '/ESRCC/ADM' ID 'ACTVT' FIELD '16'.
    is_authorised = COND #( WHEN sy-subrc = 0 THEN abap_true ELSE abap_false ).
  ENDMETHOD.
