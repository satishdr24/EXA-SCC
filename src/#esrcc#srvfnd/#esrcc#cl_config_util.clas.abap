CLASS /esrcc/cl_config_util DEFINITION PUBLIC
  ...
  PUBLIC SECTION.
    ...
    METHODS set_invalid_group_set
      IMPORTING
        linked_fields TYPE tt_linked_fields
        entity        TYPE any
        skip_first    TYPE abap_boolean OPTIONAL.

    METHODS foreign_check_ccode
      IMPORTING
    ...
    METHODS foreign_check_cost_center
      IMPORTING
        sysid         TYPE /esrcc/sysid
        ccode         TYPE /esrcc/ccode_de
        legal_entity  TYPE /esrcc/legalentity
    ...
    METHODS foreign_check_cost_element
      IMPORTING
        sysid         TYPE /esrcc/sysid
    ...
    CLASS-METHODS:
      ...
      is_editable_in_production
        IMPORTING
          tech_id         TYPE /esrcc/smbc_technical_id
        RETURNING
          VALUE(editable) TYPE abap_bool,

      is_transport_mandatory
        IMPORTING
          tech_id          TYPE /esrcc/smbc_technical_id
        RETURNING
          VALUE(mandatory) TYPE abap_bool.
...
*   If Cost Object is used in Service Consumption, Capacity or Allocation Key apps, the requested action is restricted.
  ...
  METHOD foreign_check_cost_element.
    SELECT SINGLE @abap_true
      FROM /esrcc/cstelemnt
      WHERE sysid        = @sysid
        AND cost_element = @cost_element
      INTO @DATA(found).
...
*   Entity 2
  ...
  METHOD set_invalid_group_set.
    set_state_area( state_area = CONV #( invalid_uuid ) ).

    IF skip_first = abap_false.
      set_first_flag( ).
    ENDIF.

    gt_linked_fields = linked_fields.

    gs_active_field-fieldname = gt_linked_fields[ 1 ].
    set_message_for_field(
      EXPORTING
        entity = entity
        field  = gs_active_field
        msg    = NEW cl_abap_behv( )->new_message( id = c_config_msg number = '037' severity = cl_abap_behv=>ms-error ) ).

    CLEAR gt_linked_fields.
  ENDMETHOD.

  METHOD is_editable_in_production.
    editable =
      SWITCH #( tech_id
        WHEN '/ESRCC/COSTELMENETCHARACTE'
          OR '/ESRCC/HIER1'
          OR '/ESRCC/SRTYPE'
          OR '/ESRCC/CORULE'
          OR '/ESRCC/SRVPRO'
