CLASS /esrcc/cl_config_ve_handler DEFINITION
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      tt_hier1          TYPE STANDARD TABLE OF /esrcc/c_hier1 WITH DEFAULT KEY,
      tt_ce_char        TYPE STANDARD TABLE OF /esrcc/c_costelmenetcharacte WITH DEFAULT KEY,
      tt_allocation_key TYPE STANDARD TABLE OF /esrcc/c_allocationkey WITH DEFAULT KEY,
      tt_roy_key        TYPE STANDARD TABLE OF /esrcc/c_roykey WITH DEFAULT KEY.
    ...
    METHODS hier1_update_comment
      CHANGING ct_hier TYPE tt_hier1.
    METHODS ce_char_update_comment
      CHANGING ct_ce_char TYPE tt_ce_char.
    METHODS allocation_key_update_comment
      CHANGING ct_allocation_key TYPE tt_allocation_key.
    METHODS roy_key_update_comment
      CHANGING ct_roy_key TYPE tt_roy_key.
...
CLASS /esrcc/cl_config_ve_handler IMPLEMENTATION.
  ...
  METHOD if_sadl_exit_calc_element_read~calculate.
    CASE gv_entity.
      ...
      WHEN '/ESRCC/C_HIER1'.
        DATA(lt_hier) = CORRESPONDING tt_hier1( it_original_data ).

        hier1_update_comment(
          CHANGING
            ct_hier = lt_hier
        ).

        ct_calculated_data = CORRESPONDING #( lt_hier ).

      WHEN '/ESRCC/C_COSTELMENETCHARACTE'.
        DATA(lt_ce_char) = CORRESPONDING tt_ce_char( it_original_data ).

        ce_char_update_comment(
          CHANGING
            ct_ce_char = lt_ce_char
        ).

        ct_calculated_data = CORRESPONDING #( lt_ce_char ).

      WHEN '/ESRCC/C_ALLOCATIONKEY'.
        DATA(lt_allocation_key) = CORRESPONDING tt_allocation_key( it_original_data ).

        allocation_key_update_comment(
          CHANGING
            ct_allocation_key = lt_allocation_key
        ).

        ct_calculated_data = CORRESPONDING #( lt_allocation_key ).

      WHEN '/ESRCC/C_ROYKEY'.
        DATA(lt_roy_key) = CORRESPONDING tt_roy_key( it_original_data ).

        roy_key_update_comment(
          CHANGING
            ct_roy_key = lt_roy_key
        ).

        ct_calculated_data = CORRESPONDING #( lt_roy_key ).
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD hier1_update_comment.
    ASSIGN ct_hier[ 1 ] TO FIELD-SYMBOL(<fs_hier>).

    IF <fs_hier>-commentid IS INITIAL.
      " Comment ID is not getting passed, hence retrieve it from table
      SELECT SINGLE comment_id
        FROM /esrcc/hier1
        WHERE hierarchy = @<fs_hier>-hierarchy
        INTO @<fs_hier>-commentid.
    ENDIF.

    <fs_hier>-comments = generate_comments( <fs_hier>-commentid ).
  ENDMETHOD.

  METHOD ce_char_update_comment.
    ASSIGN ct_ce_char[ 1 ] TO FIELD-SYMBOL(<fs_ce_char>).

    IF <fs_ce_char>-commentid IS INITIAL.
      " Comment ID is not getting passed, hence retrieve it from table
      SELECT SINGLE comment_id
        FROM /esrcc/cstelmtch
        WHERE cst_elmnt_char_uuid = @<fs_ce_char>-cstelmntcharuuid
        INTO @<fs_ce_char>-commentid.
    ENDIF.

    <fs_ce_char>-comments = generate_comments( <fs_ce_char>-commentid ).
  ENDMETHOD.

  METHOD allocation_key_update_comment.
    ASSIGN ct_allocation_key[ 1 ] TO FIELD-SYMBOL(<fs_allocation_key>).

    IF <fs_allocation_key>-commentid IS INITIAL.
      " Comment ID is not getting passed, hence retrieve it from table
      SELECT SINGLE comment_id
        FROM /esrcc/allockeys
        WHERE allocationkey = @<fs_allocation_key>-allocationkey
        INTO @<fs_allocation_key>-commentid.
    ENDIF.

    <fs_allocation_key>-comments = generate_comments( <fs_allocation_key>-commentid ).
  ENDMETHOD.

  METHOD roy_key_update_comment.
    ASSIGN ct_roy_key[ 1 ] TO FIELD-SYMBOL(<fs_roy_key>).

    IF <fs_roy_key>-commentid IS INITIAL.
      " Comment ID is not getting passed, hence retrieve it from table
      SELECT SINGLE comment_id
        FROM /esrcc/roykey
        WHERE royalty_base_key = @<fs_roy_key>-royaltybasekey
        INTO @<fs_roy_key>-commentid.
    ENDIF.

    <fs_roy_key>-comments = generate_comments( <fs_roy_key>-commentid ).
  ENDMETHOD.
