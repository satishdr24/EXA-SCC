...
CLASS lhc_directallocationconsumptio DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS setuuid FOR DETERMINE ON SAVE
      IMPORTING keys FOR directallocationconsumption~setuuid.
...
*   Validate UUID (Receiver)
    DATA(receiver_uuid) = /esrcc/cl_uuid_finder=>cost_center_uuid(
      EXPORTING
        parameter = CORRESPONDING #( entity MAPPING legalentity = receivingentity
                                                    companycode = receivingcompany )
    ).
    IF receiver_uuid IS INITIAL.
      lo_config_util->set_invalid_group_set(
        linked_fields = VALUE #( ( 'SYSID' )
                                 ...
                                 ( 'COSTCENTER' ) )
        entity        = entity ).
      DATA(invalid_uuid) = abap_true.
    ENDIF.

*   Validate UUID (Provider)
    DATA(provider_uuid) = /esrcc/cl_uuid_finder=>cost_center_uuid(
      EXPORTING
        parameter = CORRESPONDING #( entity MAPPING sysid       = providersysid
                                                    legalentity = providerentity
                                                    companycode = providercompany
                                                    costobject  = providercostobject
                                                    costcenter  = providercostcenter )
    ).
    IF provider_uuid IS INITIAL.
      lo_config_util->set_invalid_group_set(
        linked_fields = VALUE #( ( 'PROVIDERSYSID' )
                                 ( 'PROVIDERENTITY' )
                                 ( 'PROVIDERCOMPANY' )
                                 ( 'PROVIDERCOSTOBJECT' )
                                 ( 'PROVIDERCOSTCENTER' ) )
        entity     = entity
        skip_first = COND #( WHEN invalid_uuid = abap_true THEN abap_true ELSE abap_false ) ).
      invalid_uuid = abap_true.
    ENDIF.

    IF invalid_uuid = abap_true.
      RETURN.
    ENDIF.
...
*   Validate if provider & receiver are same
    IF receiver_uuid = provider_uuid.
...
*   Check Authorization
  ...
  METHOD setuuid.
    ...
    MODIFY ENTITIES OF /esrcc/i_diralocconsumptn IN LOCAL MODE
      ENTITY directallocationconsumption
      UPDATE FIELDS ( costobjectuuid providercostobjectuuid )
      WITH VALUE #( FOR entity IN entities ( %tky = entity-%tky
                                             costobjectuuid = /esrcc/cl_uuid_finder=>cost_center_uuid(
                                                         EXPORTING
                                                            parameter = CORRESPONDING #( entity MAPPING legalentity = receivingentity
                                                                                                        companycode = receivingcompany ) )
                                             providercostobjectuuid = /esrcc/cl_uuid_finder=>cost_center_uuid(
                                                                 EXPORTING
                                                                    parameter = CORRESPONDING #( entity MAPPING sysid       = providersysid
                                                                                                                legalentity = providerentity
                                                                                                                companycode = providercompany
    ...
    reported-directallocationconsumption = CORRESPONDING #( ureported-directallocationconsumption ).
  ENDMETHOD.
