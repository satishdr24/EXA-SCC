...
CLASS lhc_indirectallocationkeyvalue DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS setuuid FOR DETERMINE ON SAVE
      IMPORTING keys FOR indirectallocationkeyvalues~setuuid.
...
*   Validate UUID
    IF /esrcc/cl_uuid_finder=>cost_center_uuid(
      EXPORTING
        parameter = CORRESPONDING #( entity )
    ) IS INITIAL.
...
*   Check Authorization
  ...
  METHOD setuuid.
    ...
    MODIFY ENTITIES OF /esrcc/i_indirectallockeyvalue IN LOCAL MODE
      ENTITY indirectallocationkeyvalues
    ...
    reported-indirectallocationkeyvalues = CORRESPONDING #( ureported-indirectallocationkeyvalues ).
  ENDMETHOD.
