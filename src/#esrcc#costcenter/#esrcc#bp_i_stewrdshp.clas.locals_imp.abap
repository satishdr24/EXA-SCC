...
*     Validate UUID
      IF /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( t_entity ) ) IS INITIAL.
        lo_stewardship->set_invalid_group_set(
          linked_fields = VALUE #( ( 'SYSID' )
                                   ( 'LEGALENTITY' )
                                   ( 'COMPANYCODE' )
                                   ( 'COSTOBJECT' )
                                   ( 'COSTCENTER' ) )
          entity        = CORRESPONDING ts_stewardship( t_entity ) ).
      ENDIF.
...
**     END: TEMPORARY VALIDATION
  ...
  METHOD precheck_cba_receiver.
    ...
    LOOP AT target_entities INTO DATA(entity) GROUP BY ( costobjectuuid = entity-costobjectuuid
                                                         size = GROUP SIZE )
        ASCENDING REFERENCE INTO DATA(group_ref).

      DATA(tentity) = target_entities[ costobjectuuid = group_ref->costobjectuuid ].
*     Validate UUID
      IF /esrcc/cl_uuid_finder=>cost_center_uuid( parameter = CORRESPONDING #( tentity ) ) IS INITIAL.
        lo_receiver->set_invalid_group_set(
          linked_fields = VALUE #( ( 'SYSID' )
                                   ( 'LEGALENTITY' )
                                   ( 'COMPANYCODE' )
                                   ( 'COSTOBJECT' )
                                   ( 'COSTCENTER' ) )
          entity        = CORRESPONDING ts_receiver( tentity ) ).
        CONTINUE.
      ENDIF.
...
CLASS lhc_rap_tdat_cts DEFINITION.
  PUBLIC SECTION.
    ...
    CLASS-DATA:
        c_tech_id TYPE /esrcc/smbc_technical_id VALUE '/ESRCC/STEWRDSHP'.
ENDCLASS.
...
CLASS lhc_/esrcc/i_stewrdshp DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    METHODS setuuid FOR DETERMINE ON SAVE
      IMPORTING keys FOR stewardship~setuuid.
...
*        sharecost_sum = DATA(sharecost_sum) ).
    ...
    LOOP AT entities INTO DATA(entity).
      ...
      lo_stewardship->reset_state_area( entity = entity state_area = CONV #( /esrcc/cl_config_util=>overlapping_validity ) ).   " Reset State Area
...
*                                             OR   validto NOT BETWEEN entity-validfrom AND entity-validto ).
        ...
        IF product-validfrom NOT BETWEEN entity-validfrom AND entity-validto OR
           product-validto NOT BETWEEN entity-validfrom AND entity-validto.
        ...
        ELSE.
          lo_service_product->reset_state_area( entity = service_product state_area = CONV #( /esrcc/cl_config_util=>date_out_of_range ) ).   " Reset State Area
        ENDIF.
      ENDLOOP.
  ...
  METHOD triggerworkflow.
    ...
    DATA(lo_wf_handler) = NEW /esrcc/cl_wf_handler_std( application_type = /esrcc/cl_wf_utility=>c_app-bc_stewardship ).
...
*   Pre-check validation before create
  ...
  METHOD setuuid.
    ...
    MODIFY ENTITIES OF /esrcc/i_stewrdshp_s IN LOCAL MODE
      ENTITY stewardship
      UPDATE FIELDS ( costobjectuuid )
      WITH VALUE #( FOR entity IN entities ( %tky = entity-%tky
                                             costobjectuuid = /esrcc/cl_uuid_finder=>cost_center_uuid( EXPORTING parameter = CORRESPONDING #( entity ) ) ) )
      FAILED DATA(ufailed)
      REPORTED DATA(ureported).

    reported-stewardship = CORRESPONDING #( ureported-stewardship ).
  ENDMETHOD.
...
CLASS lhc_serviceproduct IMPLEMENTATION.
  ...
  METHOD validatedata.
    ...
    READ ENTITIES OF /esrcc/i_stewrdshp_s IN LOCAL MODE
         ...
         ENTITY serviceproduct
         BY \_stewardship
         FIELDS ( stewardshipuuid validfrom validto ) WITH CORRESPONDING #( keys )
         RESULT DATA(stewardships).

    READ ENTITIES OF /esrcc/i_stewrdshp_s IN LOCAL MODE
      ENTITY stewardship
      BY \_serviceproduct
      ALL FIELDS WITH CORRESPONDING #( stewardships )
      RESULT DATA(prod_entities_all).
    ...
    lo_validation->calculate_costshare_sum(
      EXPORTING
        sharecost     = CORRESPONDING #( prod_entities_all MAPPING service_product_uuid = serviceproductuuid
...
CLASS lhc_servicereceiver DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    METHODS setuuid FOR DETERMINE ON SAVE
      IMPORTING keys FOR servicereceiver~setuuid.
...
CLASS lhc_servicereceiver IMPLEMENTATION.
  ...
  METHOD setuuid.
    ...
    reported-servicereceiver = CORRESPONDING #( ureported-servicereceiver ).
  ENDMETHOD.
